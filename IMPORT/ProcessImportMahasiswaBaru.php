...
//cek data id_pd ada tidak, kalau ada -> insert id_reg_pd, kalau tidak ada -> insert id_pd, id_reg_pd	

$filter_id_pd = "nm_pd ilike '%".$nm_pd."%' and tmpt_lahir ilike '%".$tmpt_lahir."%' and tgl_lahir = '".$tgl_lahir."' and nm_ibu_kandung ilike '%".$nm_ibu_kandung."%'";
$temp_id_pd = $proxy->GetRecord($token,'mahasiswa',$filter_id_pd);
if($temp_id_pd['result']){
		$id_pd = $temp_id_pd['result']['id_pd'];	
}
...
if($id_pd!=''){
	...
	$insert_pt = $proxy->InsertRecord($token,$table1,json_encode($data_pt));
		//var_dump($insert_pt);
			if($insert_pt['result']['error_desc']==NULL){
				$LOG=$LOG.'[SUCCESS] Ada data mahasiwa. input data mahasiswa_pt. Data "'.$nm_pd.' / '.$nipd.'" berhasil di tambahkan <br>';
				$REPORT = '[SUCCESS] Ada data mahasiwa. input data mahasiswa_pt. Data "'.$nm_pd.' / '.$nipd.'" berhasil di tambahkan <br>';
			}
			else{
				$LOG=$LOG."[Error] Ada data mahasiwa. input data mahasiswa_pt.  pada data ".$nm_pd." / ".$nipd." - ".$insert_pt['result']['error_desc']."<br>";	
				$REPORT = "[Error] Ada data mahasiwa. input data mahasiswa_pt. pada data ".$nm_pd." / ".$nipd." - ".$insert_pt['result']['error_desc']."<br>";				
				}
	//echo "ada";	
}else{
	...
	if($insert['result']['error_desc']==NULL){
		$LOG=$LOG.'[SUCCESS] Tidak ada data mahasiwa. input data mahasiswa. Data "'.$nm_pd.' / '.$nipd.'" berhasil di tambahkan <br>';
		$REPORT = '[SUCCESS] Tidak ada data mahasiwa. input data mahasiswa. Data "'.$nm_pd.' / '.$nipd.'" berhasil di tambahkan <br>';
	}else{
		$LOG=$LOG."[Error] Tidak ada data mahasiwa. input data mahasiswa. Data ".$nm_pd." / ".$nipd." - ".$insert['result']['error_desc']."<br>";	
		$REPORT = "[Error] Tidak ada data mahasiwa. input data mahasiswa. Data ".$nm_pd." / ".$nipd." - ".$insert['result']['error_desc']."<br>";		
	}
	
	//cari id_pd yang baru saja masuk
	$filter_id_pd = "nm_pd ilike '%".$nm_pd."%' and tmpt_lahir ilike '%".$tmpt_lahir."%' and tgl_lahir = '".$tgl_lahir."' and nm_ibu_kandung ilike '%".$nm_ibu_kandung."%'";
	$temp_id_pd = $proxy->GetRecord($token,'mahasiswa',$filter_id_pd);
	if($temp_id_pd['result']){
		$id_pd = $temp_id_pd['result']['id_pd'];	
	...
	//cari id_sms yang baru saja masuk
	$filter_sms = "id_sp='".$id_sp."' and kode_prodi ilike '%".$kode_prodi."%'";
	$temp_sms = $proxy->GetRecord($token,'sms',$filter_sms);
	if($temp_sms['result']){
		$id_sms = $temp_sms['result']['id_sms'];	
	...
	$insert_pt = $proxy->InsertRecord($token,$table1,json_encode($record_pt));
		//var_dump($insert_pt);
			if($insert_pt['result']['error_desc']==NULL){
				$LOG=$LOG.'[SUCCESS] Tidak ada data mahasiwa. input data mahasiswa_pt. Data "'.$nm_pd.' / '.$nipd.'" berhasil di tambahkan <br>';
				$REPORT = '[SUCCESS] Tidak ada data mahasiwa. input data mahasiswa_pt. Data "'.$nm_pd.' / '.$nipd.'" berhasil di tambahkan <br>';
			}
			else{
				$LOG=$LOG."[Error] Tidak ada data mahasiwa. input data mahasiswa_pt. Data ".$nm_pd." / ".$nipd." - ".$insert_pt['result']['error_desc']."<br>";	
				$REPORT = "[Error] Tidak ada data mahasiwa. input data mahasiswa_pt. Data ".$nm_pd." / ".$nipd." - ".$insert_pt['result']['error_desc']."<br>";				
				}
